...
protected:
    ...
    virtual void testCaseOnStart(void)
    {
        ...
        zfLangLoad(ZFSerializableDataFromXml(ZFInputCallbackForResFile(zfText("test_ZFLang_default.xml"))));
        this->test();

        this->testCaseOutputSeparator();
        this->testCaseOutput(zfText("load custom language"));
        zfLangLoad(ZFSerializableDataFromXml(ZFInputCallbackForResFile(zfText("test_ZFLang_custom.xml"))));
        this->test();
