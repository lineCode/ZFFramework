                       ...
                       ZFMP_IN(ZFUIWindow *, window))
{
    ...
    window->tagSet(_ZFP_I_ZFUIOnScreenKeyboardAutoResizeTaskData::ClassData()->className(), taskDataTmp);
...
ZF_GLOBAL_INITIALIZER_DESTROY(ZFUIOnScreenKeyboardAutoResizeDataHolder)
{
    zfCoreAssertWithMessageTrim(this->windowList.isEmpty(),
        zfTextA("ZFUIOnScreenKeyboardAutoResizeStart/ZFUIOnScreenKeyboardAutoResizeStop mismatch, have you forgot to stop?"));
}
public:
...
ZF_GLOBAL_INITIALIZER_DESTROY(ZFUIOnScreenKeyboardAutoResizeAutoStop)
{
    ...
    {
        d->windowList.getLast()->tagRemove(_ZFP_I_ZFUIOnScreenKeyboardAutoResizeTaskData::ClassData()->className());
    }
}
